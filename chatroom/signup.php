...
<head>
  <meta charset="utf-8" />
  <title>MyChatHub | Join Now For Free</title>
...
<meta property="og:title" content="MyChatHub - Join Now For Free">
...
<meta property="og:url" content="https://www.mychathub.in/chatroom/signup.php">
...
<!-- ############ LAYOUT START-->
  <div class="center-block w-xxl w-auto-xs p-y-md">
    ...
    <div class="p-a-md box-color r box-shadow-z1 text-color m-a">
      ...
      <form name="signup-form" id='signup-form' method='post' action='creat-account.php'>
        <div class="md-form-group float-label">
          <input type="text" name='username' id='username' maxlength='30' class="md-input form-control" ng-model="user.text" autocomplete='off' required>
        ...
        <div class="md-form-group float-label">
          <input type="email" name='email' id='email' maxlength='60' class="md-input form-control" autocomplete='off' ng-model="user.email" required>
          <small class='text-muted' style='font-size:10px;'>* example@example.com | Not already linked.</small>
		  <label>Email</label>
		 
        </div>
        <div class="md-form-group float-label">
          <input type="password" name='password' id='password' maxlength='120' class="md-input form-control" autocomplete='off' ng-model="user.password" required>
          <small class='text-muted' style='font-size:10px;'>* Min 8 chars.</small>
    ...
    <div class="p-v-lg text-center">
      <div>Already have an account? <a ui-sref="access.signin" href="index.php" class="text-primary _600">Sign in</a>
...
var username_length = $('#username').val().length;
           $.post('validate-username.php', {username: now_input_value}, function(output1){ if(now_input_value!='' &&output1==0 && username_length>=3 && username_length<=30 && /^[a-zA-Z0-9_ ]*$/.test(now_input_value) ){$('#username').removeClass('is-invalid');$('#username').addClass('is-valid');} else{$('#username').removeClass('is-valid');$('#username').addClass('is-invalid');}});
 });
...
var email_length = $('#email').val().length;
           $.post('validate-email.php', {email: email_input}, function(output2){if(email_input!='' && output2==0 && email_length<=60 &&email_pattern.test(email_input)){$('#email').removeClass('is-invalid');$('#email').addClass('is-valid');} else{$('#email').removeClass('is-valid');$('#email').addClass('is-invalid');}});
...
var pass_len = $('#password').val().length;
            if(pass!='' && pass_len>=8 && pass_len<=120){$('#password').removeClass('is-invalid');$('#password').addClass('is-valid');} else{$('#password').removeClass('is-valid');$('#password').addClass('is-invalid');};
 });
		   
  var check=function(){if( $('#username').hasClass('is-valid') && $('#email').hasClass('is-valid') && $('#password').hasClass('is-valid')&& $('#tp_link').hasClass('is-valid') && $('#loader-spin').is(':hidden')){$('#submit-btn').prop('disabled',false);$('#submit-btn').addClass('primary');}
